...
export async function createStudyMaterial(
  ...
  try {
    ...
    // السنة الدراسية الحالية — مطلوبة لأن StudyMaterial يربطها
    const academicYear = await getCurrentAcademicYearOrNull(teacher.schoolId);
    if (!academicYear) {
      return {
        success: false,
        error: true,
    ...
    const subject = await prisma.subject.findFirst({
      where: { id: parsed.data.subjectId, schoolId: teacher.schoolId },
    });
    if (!subject) {
